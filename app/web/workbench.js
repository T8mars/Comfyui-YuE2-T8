(() => {
  ...
  async function renderTrainingModel(run=currentRun){
    const panel=$('#training-model-manager');
    if(!run?.model_asset_id){panel.classList.add('hidden');panel.innerHTML='';return;}
    const runId=run.id;panel.classList.remove('hidden');panel.innerHTML='<p class="meta">正在读取训练模型信息…</p>';
    try{
      const artifacts=await api(`/api/workbench/training-runs/${encodeURIComponent(runId)}/artifacts`);
      if(currentRun?.id!==runId)return;
      const model=artifacts.model,meta=model?.metadata||{},history=run.config?.history||[],selectedStep=Number(meta.selected_validation_step??run.config?.selected_step??0),selectedPoint=history.find(item=>Number(item.step)===selectedStep)||history.at(-1)||{},metric=value=>value!==null&&value!==undefined&&Number.isFinite(Number(value))?Number(value).toFixed(3):'—';
      if(!model){panel.classList.add('hidden');panel.innerHTML='';return;}
      panel.dataset.modelPath=model.path||'';
      const filename=(String(model.title||'YuE2-style-model').replace(/[<>:"/\\|?*\x00-\x1f]+/g,'_').trim()||'YuE2-style-model')+(model.suffix||'.safetensors');
      panel.innerHTML=`<h4><i class="bi bi-gpu-card"></i> 已完成模型 · ${escapeHtml(model.title)}</h4><p class="meta">模型已保存，可直接用于歌曲创作，也可以下载备份。</p><div class="training-model-grid"><span><small>完成训练</small><b>${Number(meta.completed_training_steps??run.config?.completed_training_steps??run.config?.steps??0)} 步</b></span><span><small>采用检查点</small><b>第 ${selectedStep||'—'} 步</b></span><span><small>训练 loss</small><b>${metric(selectedPoint.train_loss)}</b></span><span><small>验证 loss</small><b>${metric(meta.selected_validation_loss??selectedPoint.validation_loss)}</b></span><span><small>LoRA rank</small><b>${Number(meta.rank??run.config?.rank??0)||'—'}</b></span><span><small>模型大小</small><b>${(Number(model.size)/1048576).toFixed(1)} MB</b></span><span><small>生成模式</small><b>直接生成</b></span><span><small>强度建议</small><b>从 0.2–0.4 试听</b></span></div><p class="meta">模型文件位置（资产库内部使用哈希名，下载文件会使用 .safetensors）</p><div class="training-model-path">${escapeHtml(model.path)}</div><div class="toolbar"><button class="primary" data-use-trained-model="${escapeHtml(model.id)}">用于歌曲创作</button><a class="ghost" href="/api/workbench/assets/${encodeURIComponent(model.id)}/content?revision_id=${encodeURIComponent(model.revision_id)}" download="${escapeHtml(filename)}">下载模型</a><button class="ghost" type="button" data-open-trained-model>打开文件夹</button><button class="ghost compact" type="button" data-copy-trained-model-path>复制位置</button></div>`;
    }catch(error){if(currentRun?.id===runId){panel.classList.remove('hidden');showError(panel,error);}}
  }
  function renderRun(){
  ...
  document.addEventListener('click',async event=>{
    ...
    if(event.target.closest('[data-open-trained-model]')&&currentRun){try{await api(`/api/workbench/training-runs/${encodeURIComponent(currentRun.id)}/open`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({target:'model'})});}catch(error){alert(error.message);}}
    if(event.target.closest('[data-copy-trained-model-path]')){const path=$('#training-model-manager').dataset.modelPath||'';try{await navigator.clipboard.writeText(path);event.target.closest('button').textContent='已复制位置';}catch{alert(path);}}
  });
