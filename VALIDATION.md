...
目标设备：RTX 5090 Laptop 24GB，Windows，统一 CPython 3.12.10、Torch 2.10.0+cu128、NumPy 1.26.4、Transformers 4.57.6。以下是开发候选的已完成验证；完整包冷启动与最终发布验收另行补充，不能视为已经发布。
...
限制：真实多歌手、跨音域、气声和盲听质量矩阵未完成；训练 OOM 自动降批机制已实现，真实 OOM 触发恢复尚待验证。不把兼容性测试写成音质排名或低显存硬件承诺。
