...
class HeartCodec(PreTrainedModel):
    ...
    def detokenize(
        ...
        segment_starts = list(range(0, codes.shape[-1] - hop_samples + 1, hop_samples))
        for segment_index, sinx in enumerate(segment_starts):
            if cancelled is not None and cancelled():
                raise InterruptedError("MuLaCover decode was cancelled")
            codes_input = []
            ...
            if on_progress is not None:
                on_progress("decoding", segment_index + 1, max(1, len(segment_starts)))
