...
分批加载使用整合包已有的 Accelerate，不需要额外 Python 或模型；会显著减慢生成，建议至少 32 GB 内存。显存预算包含 2 GiB 预留，例如填 8 时进程上限约 6 GiB；调低预算本身不会缩小模型。已在 RTX 5090 Laptop 上限制进程分配为 6 / 4 GiB，验证原始模型生成、声学合成和音频解码，固定输入的短片段与常规模式输出一致。该验证不代表已完成 RTX 3060 Ti 等原生 8 GB 显卡的长歌验收，长曲仍受缓存、其他程序占用和硬件速度影响。这个选项不控制 RVC 训练、直接换声或 MuLaCover。
