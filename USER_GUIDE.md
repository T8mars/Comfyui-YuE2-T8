...
2. 为每首含人声歌曲选择对应歌词，可选逐首曲风；纯器乐必须逐首勾选。再选择快速、标准或“较长试训 1600”方案，勾选素材使用权确认并点“创建快照并预处理”。1600 步只代表训练更久，不保证质量更高，应根据验证 loss 和固定样例试听选择检查点。系统固定每个素材版本、片段、集合和确认记录，之后资产更新不会改变本次数据集；相同音频即使重复导入，也不能跨训练集和验证集。
...
首期训练的是 YuE2 AR LoRA，用于歌曲风格，不是歌手音色克隆；专用演唱音色仍由 RVC 训练。已训练模型自动关联固定 v4 NAR companion，目前只在“直接生成”模式启用。短试听只用于快速判断模型是否正常，最终选择应使用固定提示与种子的完整 A/B 试听。
