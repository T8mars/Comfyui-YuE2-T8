...
YuE2 歌曲风格训练只显示当前歌曲项目中的歌曲。若资产库已有音频但训练页为空，点击“添加歌曲”，再点歌曲卡片上的“加入当前项目供训练”；“已加入项目 · 训练可用”表示返回训练页后会显示。也可以在训练空状态中直接导入歌曲。每首含人声歌曲默认会展开“直接粘贴本曲歌词”，也可选择项目里的歌词资产；右侧公共歌词仅是备用。创建按钮旁会直接提示缺少的曲风、歌词、验证集或授权，并在预处理期间显示后台状态。
...
新任务默认由当前 LLM 一并创作 ABC。完整乐谱通常比文本耗时更长，未通过检查时最多修正一次；失败会保留已完成的词和曲风。已有结果显示“ABC 留空”时，直接点击“补写 ABC”，系统只重试作谱，不会重新生成歌词和曲风。需要减少 API 调用时仍可选择交给下游 YuE2 规划。
