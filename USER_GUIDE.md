...
1. v1.4.x 完整整合包已经带好音乐模型、RVC 底模、YuE2 训练资源和一个共享的 Python 3.12.10 运行环境，本地 GGUF 后端也在同一环境中（GGUF 权重可选下载），直接双击 `YuE2-T8.exe`，浏览器会打开 `http://127.0.0.1:8189`。如果系统拦截 EXE，也可双击兼容入口 `启动本地整合包.bat`。
