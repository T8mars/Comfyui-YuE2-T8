...
更新不会删除模型、Python 运行环境、本地 GGUF、作品、上传文件、导出、日志、缓存、助手草稿和设置。有任务正在运行或排队时，先等待任务结束。每次替换前的代码备份位于 `logs/backups`；如果新版服务启动失败，更新器会自动恢复旧代码。

v1.2.0 / v1.2.1 没有这个页面入口，需要先手动安装一次 v1.2.2 代码更新；以后即可使用页面按钮。
...
使用 API 无需下载或设置本地 LLM。[本地 LLM 模型下载（可选）](https://pan.quark.cn/s/55eab3bb2d9b)：解压后可填写存放 GGUF 的目录，例如 `E:\LLM`；目录不是必填项，留空时默认扫描模型根目录的 `LLM` 文件夹。先运行整合包中的 `安装本地LLM.bat` 安装独立环境，再选择模型并测试连接。分片模型选择第一片，其他分片必须齐全；不需要视觉投影模型。目录列表会显示可读取的架构、上下文和分片信息。模型权重需要自己准备，创作按钮不会自动下载。 实测 Qwen3.8-27B-Q4_K_M 的歌词与曲风生成通过；本次直接生成 ABC 的样例未通过时值校验，因此默认仍由 YuE2 规划乐谱。本地模型的作谱能力需按实际输出判断。
