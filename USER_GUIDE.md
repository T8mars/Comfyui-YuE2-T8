...
先选择渠道：贞贞平价小屋、贞贞的 AI 工坊、OpenAI 兼容接口或本地 GGUF。API 模型名按渠道实际支持的名称填写；兼容接口支持 HTTPS，以及本机 HTTP 服务。填写密钥后保存设置，默认只在本次服务会话有效；勾选“记住”才使用 Windows 当前用户加密保存。任务和导出文件只保存凭据引用。关闭服务后，未记住的密钥需要重新填写。
