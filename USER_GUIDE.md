...
1. v1.4.0 完整整合包已经带好音乐模型、RVC 底模、YuE2 训练资源和一个共享的 Python 3.12.10 运行环境，本地 GGUF 后端也在同一环境中（GGUF 权重可选下载），直接双击 `YuE2-T8.exe`，浏览器会打开 `http://127.0.0.1:8189`。如果系统拦截 EXE，也可双击兼容入口 `启动本地整合包.bat`。
...
“资产库”统一保存歌曲、作品、人声、伴奏、参考音色、歌词、曲风、ABC 乐谱和模型。导入音频前先选择类型，上传完成即可试听和查看波形；加入项目时固定当前素材版本，之后修改文本或替换素材不会悄悄改变旧项目。`userdata/assets` 保存按 SHA-256 去重的文件，`userdata/library.sqlite3` 保存项目、版本和来源关系，更新器会保留两者。
...
2. 填写公共曲风、公共歌词和 LoRA 参数，勾选素材使用权确认，再点“创建快照并预处理”。系统固定每个素材版本、片段、集合和确认记录，之后资产更新不会改变本次数据集。
...
5. 完成后点“生成短试听”，在当前页面试听约 10 秒的检查结果；满意后发送到“创作”。模型会自动出现在创作页的“歌曲风格模型”下拉框。
